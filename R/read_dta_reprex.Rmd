...
# cs_wave3_varnames <- names(cs_wave3) %>% 
#   as_tibble() %>%
#   mutate(len = str_length(value),
#          new_name = case_when( len >= 32 ~ str_trunc(value, width = 31, side = "center", ellipsis = "_"),
#                                TRUE      ~ value)
#          ) %>%
#   filter(len >= 32)

cs_wave3_short <- cs_wave3 %>%
    rename_with(.cols = all_of(to_change), ~ str_trunc(.x, width = 31, side = "center", ellipsis = "_")) 
