...
#### Passing the encoding arguement
...
```
Ah yes, we will have to shorten the names of variables to 32 characters or fewer so that Stata accepts them. (Apparently all of the errors are exactly 32 characters, but I'm not willing to dig into this)
...
What's with this error? grepl thinks the `from` argument is a single regex expression instead of separate ones for each label.

```{r}
harmonized_cs_waves_A <- harmonize_waves(waves = lapply(merged_cs, function(x) x %>%
                                                          select(-.data$demo_hh_size)), 
                                         .f = harmonize_cs_edu)
```
